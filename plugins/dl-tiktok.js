...
cmd({
    ...
    try {
        ...
        // Try First API: https://NAWAZ-MD.vercel.app/download/tiktok?url=
        try {
            const api1 = `https://NAWAZ-MD.vercel.app/download/tiktok?url=${encodeURIComponent(q)}`;
            ...
            try {
                const api2 = `https://NAWAZ-MD.vercel.app/download/ttdl?url=${encodeURIComponent(q)}`;
...
cmd({
    ...
    try {
        ...
        // Fetch TikTok data
        const api = `https://NAWAZ-MD.vercel.app/download/tiktok?url=${encodeURIComponent(q)}`;
...
cmd({
    pattern: "tiktok3",
    alias: ["tt3", "ttdl3"],
    desc: "Download HD TikTok videos using NAWAZ-MD API",
    category: "download",
    react: "🎬",
    filename: __filename
}, async (conn, mek, m, { from, q, reply }) => {
    try {
        ...
        // 🌐 Fetch TikTok data from NAWAZ-MD API
        const api = `https://NAWAZMD.vercel.app/download/ttdl?url=${encodeURIComponent(q)}`;
        const res = await axios.get(api);
        const json = res.data;
